    ...
    public async Task<Result<TaskResponse>> ExecuteAsync(Guid id, UpdateTaskRequest request, CancellationToken cancellationToken = default)
{
    ...
    {
        if (string.IsNullOrWhiteSpace(request.Title.Value) || request.Title.Value.Trim().Length < 3)
        {
            return Result.Fail("O título da tarefa deve ter pelo menos 3 caracteres e não pode ser vazio.");
        }
    }

    if (request.CategoryId.IsDefined && request.CategoryId.Value.HasValue)
    {
        var category = await _categoryRepository.GetByIdAsync(request.CategoryId.Value.Value, cancellationToken);
        if (category is null)
        {
    ...
    if (request.DueDate.IsDefined && request.DueDate.Value.HasValue && request.DueDate.Value.Value < DateTime.UtcNow)
    {
